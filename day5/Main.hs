...
compute pc mem = do
  ...
  maybe (pure mem) (compute pc') =<< eval instr mem

eval :: HasCallStack => Instr -> Memory -> IO (Maybe Memory)
eval instr =
  case instr of
...
add :: HasCallStack => Value -> Value -> Int -> Memory -> IO (Maybe Memory)
add v1 v2 out mem = do
  let i1 = fetch v1 mem
      i2 = fetch v2 mem
  pure (Just $ mem // [(out, i1 + i2)])
    

mul :: HasCallStack => Value -> Value -> Int -> Memory -> IO (Maybe Memory)
mul v1 v2 out mem = do
  let i1 = fetch v1 mem
      i2 = fetch v2 mem
  pure (Just $ mem // [(out, i1 * i2)])

input :: HasCallStack => Int -> Memory -> IO (Maybe Memory)
input dest mem = do
  input <- read <$> getLine
  pure (Just $ mem // [(dest, input)])

output :: HasCallStack => Value -> Memory -> IO (Maybe Memory)
output val mem = do
  print (fetch val mem)
  pure (Just mem)

halt :: HasCallStack => Memory -> IO (Maybe Memory)
halt = const (pure Nothing)
